...
async function calcularArtilharia(campeonatoId, timeIdsDoCampeonato) {
  ...
  // Carrega todos os atletas APROVADOS dos times do campeonato para exibir
  // nome/foto/time, mesmo os que ainda não marcaram gol nenhum (não entram
  // na lista, só os artilheiros). Atletas pendentes/rejeitados nunca
  // aparecem aqui — mesmo que já tenham gol lançado em algum jogo.
  const todosAtletas = (await Promise.all(
    (timeIdsDoCampeonato || []).map((timeId) => listarAtletas({ timeId, somenteAprovados: true }))
  )).flat();
