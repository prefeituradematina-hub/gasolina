...
/**
 * Exclui o CADASTRO (perfil em /usuarios) do usuário — isso já revoga o
 * acesso ao sistema, pois o login exige que esse doc exista (ver
 * carregarPerfil em auth.js). IMPORTANTE: como este projeto não tem
 * backend/Cloud Functions, não é possível excluir a CONTA do Firebase
 * Authentication pelo SDK do cliente a partir da sessão do admin (só o
 * próprio usuário logado pode se autoexcluir do Auth). Se quiser liberar
 * o e-mail para reuso, exclua manualmente em Firebase Console >
 * Authentication depois de excluir o perfil aqui.
 *
 * Não apaga times/atletas/jogos relacionados — o time de um responsável
 * excluído continua intacto (só fica sem um usuário vinculado até o
 * admin, se quiser, vincular outro).
 */
...
export { criarUsuario, listarUsuarios, alternarBloqueio, excluirUsuario };
